// Found in: https://gist.github.com/aholkner/214628a05b15f0bb169660945ac7923b
...
public static class SerializedPropertyExtensions
{
    ...
    /// (Extension) Return the direct container of this property.
    public static object GetDirectParent(this SerializedProperty property)
    {
        var splitPath = property.propertyPath.Split('.');
        if(splitPath.Length >= 3 && splitPath[splitPath.Length - 2] == "Array")
            splitPath = splitPath.SkipLast(2).ToArray();
        else
            splitPath = splitPath.SkipLast(1).ToArray();
        var removedPath = string.Join('.', splitPath);

        string propertyPath = removedPath;
        object value = property.serializedObject.targetObject;
        int i = 0;
        while (NextPathComponent(propertyPath, ref i, out var token))
            value = GetPathComponentValue(value, token);
